...
class Login {
	...
	public function __construct() {
		...
		if ($this -> connection) {
			...
			if (isset($_POST["aveshan_register"])) {
			...
			} elseif (!empty($_SESSION['aveshan_user_name']) && ($_SESSION['user_logged_in'] == 1)) {

				$this -> loginWithSessionData();

			} elseif (isset($_POST["aveshan_signin"])) {

				if (!empty($_POST['aveshan_user_name']) && !empty($_POST['aveshan_user_password'])) {

					$this -> loginWithPostData();

				} elseif (empty($_POST['aveshan_user_name'])) {

					$this -> errors[] = "Username field was empty.";

				} elseif (empty($_POST['aveshan_user_password'])) {
		...
		if (isset($_COOKIE['aveshan_user_name'])) {
			$this -> view_user_name = strip_tags($_COOKIE["aveshan_user_name"]);
		} else {
	...
	private function loginWithPostData() {
		...
		$this -> user_name = $this -> connection -> real_escape_string($_POST['aveshan_user_name']);
		...
		if ($checklogin -> num_rows == 1) {
			...
			if (crypt($_POST['aveshan_user_password'], $result_row -> user_password_hash) == $result_row -> user_password_hash) {

				/**
				 *  write user data into PHP SESSION [a file on your server]
				 */
				$_SESSION['aveshan_user_name'] = $result_row -> user_name;
				$_SESSION['aveshan_user_email'] = $result_row -> user_email;
				$_SESSION['user_logged_in'] = 1;

				/**
				 *  write user data into COOKIE [a file in user's browser]
				 */
				setcookie("aveshan_user_name", $result_row -> user_name, time() + (1 * 1 * 1));
				setcookie("aveshan_user_email", $result_row -> user_email, time() + (1 * 1 * 1));
